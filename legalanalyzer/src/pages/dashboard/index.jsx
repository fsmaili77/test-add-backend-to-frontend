...
const Dashboard = () => {
  ...
  const mockDocuments = [
    ...
    {
      id: 2,
      filename: 'Legal_Brief_Case_456.docx',
    ...
    {
      id: 4,
      filename: 'Patent_Application_789.pdf',
    ...
    {
      id: 5,
      filename: 'Merger_Agreement_Draft.docx',
    ...
    {
      id: 6,
      filename: 'Regulatory_Filing_2024.pdf',
      uploadDate: new Date('2024-01-10'),
      status: 'Complete',
      type: 'Filing',
      size: '1.5 MB',
      analysisProgress: 100,
      lastModified: new Date('2024-01-10T15:25:00')
    }
  ];
  ...
  const filteredDocuments = useMemo(() => {
    if (activeFilter === 'All') return mockDocuments;
    return mockDocuments.filter(doc => doc.status === activeFilter);
  }, [activeFilter]);
  ...
  const filterTabs = [
    { key: 'All', label: texts.allDocuments, count: mockDocuments.length },
    { key: 'Processing', label: texts.processing, count: mockDocuments.filter(d => d.status === 'Processing').length },
    { key: 'Complete', label: texts.complete, count: mockDocuments.filter(d => d.status === 'Complete').length },
    { key: 'Failed', label: texts.failed, count: mockDocuments.filter(d => d.status === 'Failed').length }
  ];
