// src/pages/service-plus/components/ComparisonHistory.jsx

import React, { useState, useEffect, useCallback } from 'react';
import { FileText, Download, Trash2, Search, RefreshCw, AlertCircle, AlertTriangle, History, GitCompare, File, ArrowRight, ChevronLeft, ChevronRight, X, MousePointer } from 'lucide-react';
...
const exportComparisonToPDF = async (comparison, API_BASE_URL) => {
  ...
  try {
    ...
    if (response.ok) { fullData = await response.json(); }
  } catch (err) {
...
const ComparisonHistory = ({ onStatsUpdate, backendHealth }) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          setComparisons([]); setTotalPages(1); setTotalCount(0);
          return;
        }
      ...
      if (onStatsUpdate && data.comparisons) {
        ...
        const todayComparisons = data.comparisons.filter(comp => {
          ...
          return new Date(comp.compared_at).toDateString() === today.toDateString();
        }).length;
      ...
      setError(err.message || texts.failedToLoadHistory);
      setComparisons([]);
    } finally {
  ...
  const fetchComparisonDetails = async (comparisonId) => {
    try {
      ...
      setError(texts.failedToLoadComparisonDetails);
    }
  };

  const deleteComparison = async (comparisonId) => {
    const comparison = comparisons.find(c => c.id === comparisonId);
    const confirmMessage = `${texts.confirmDeleteComparison} "${comparison?.doc1_name}" ${texts.and} "${comparison?.doc2_name}"? ${texts.thisActionCannotBeUndone}`;
    ...
    try {
      ...
      setError(texts.failedToDeleteComparison);
    }
  };
  ...
  const formatDate = (dateString) => {
    if (!dateString) return texts.unknown;
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">{texts.loadingComparisonHistory}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700">{texts.historyLoadingLimited}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={texts.searchComparisons}
              value={searchQuery}
          ...
          >
            ...
            <option value="advanced-gemini">{texts.aiAnalysis}</option>
            <option value="basic">{texts.basicComparison}</option>
          </select>
        </div>
        <div className="flex items-center space-x-2">
          <span className="text-sm text-gray-600">
            {totalCount} {texts.comparisonsTotal}
          </span>
          <button
          ...
          {filteredComparisons.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {searchQuery ? texts.noMatchingComparisons : texts.noComparisonsFound}
              </h3>
              <p className="text-gray-600">
                {searchQuery ? texts.adjustSearchTerms : texts.startComparingToSeeHistory}
              </p>
            </div>
          ) : (
            <>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-gray-900">
                            {texts.comparison} #{comparison.id}
                          </span>
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            ...
                            {comparison.comparison_type === 'advanced-gemini' ? texts.aiAnalysis : texts.basic}
                          </span>
                          {comparison.similarity_score !== undefined && (
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getSimilarityBadgeColor(comparison.similarity_score)}`}>
                              {(comparison.similarity_score * 100).toFixed(0)}% {texts.similar}
                            </span>
                          )}
                        </div>
                        <div className="space-y-1 text-sm">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-gray-900 truncate">{comparison.doc1_name || texts.document1}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            <ArrowRight size={14} className="text-gray-400 flex-shrink-0" />
                            <span className="text-gray-900 truncate">{comparison.doc2_name || texts.document2}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between mt-3">
                          <span className="text-xs text-gray-600">{formatDate(comparison.compared_at)}</span>
                        </div>
                      </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4">
              <div className="text-sm text-gray-600">
                {texts.showing} {Math.min((currentPage - 1) * 10 + 1, totalCount)} {texts.to} {Math.min(currentPage * 10, totalCount)} {texts.of} {totalCount} {texts.results}
              </div>
              <div className="flex items-center space-x-2">
                <button
        ...
        <div className="lg:col-span-1">
          {selectedComparison ? (
            <div className="bg-white rounded-lg border border-gray-200 p-4 sticky top-24">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {texts.comparison} #{selectedComparison.id}
                </h3>
                <button
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">{texts.documents}</h4>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 text-center sticky top-24">
              ...
              <h4 className="font-medium text-gray-900 mb-2">{texts.selectAComparison}</h4>
              <p className="text-gray-600 text-sm">{texts.selectAComparisonDesc}</p>
            </div>
          )}
        </div>
      </div>
