...
const ClientManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    phoneNumber: '', company: '', address: '', notes: ''
  });
  const [formErrors, setFormErrors] = useState({});

  useEffect(() => { fetchClients(); }, [searchTerm, showActiveOnly]);
  ...
  const fetchClients = async () => {
    ...
    try {
      const response = await axios.get(`${API_URL}/clientmanagement/clients`, {
        params: { search: searchTerm || undefined, isActive: showActiveOnly ? true : undefined },
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.firstName.trim()) errors.firstName = `${texts.firstName} is required`;
    if (!formData.lastName.trim()) errors.lastName = `${texts.lastName} is required`;
    if (!formData.email.trim()) {
      errors.email = `${texts.emailAddress} is required`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingClient) {
        await axios.put(`${API_URL}/clientmanagement/clients/${editingClient.id}`, formData, { headers: getAuthHeaders() });
      } else {
        await axios.post(`${API_URL}/clientmanagement/clients`, formData, { headers: getAuthHeaders() });
      }
      setShowModal(false);
      resetForm();
      fetchClients();
    } catch (err) {
      alert(err.response?.data?.error || texts.error);
    }
  };

  const resetForm = () => {
    setFormData({ firstName: '', lastName: '', email: '', phoneNumber: '', company: '', address: '', notes: '' });
    setFormErrors({});
  ...
  const handleEdit = (client) => {
    ...
    setFormData({
      ...
      phoneNumber: client.phoneNumber || '', company: client.company || '',
  ...
  const handleDelete = async (client) => {
    if (!confirm(`${texts.confirmDelete} ${client.firstName} ${client.lastName}? ${texts.thisActionCannotBeUndone}`)) return;
    try {
      await axios.delete(`${API_URL}/clientmanagement/clients/${client.id}`, { headers: getAuthHeaders() });
      fetchClients();
    } catch (err) {
      alert(err.response?.data?.error || texts.error);
    }
  ...
  const handleToggleActive = async (client) => {
    try {
      ...
      await axios.put(`${API_URL}/clientmanagement/clients/${client.id}/${endpoint}`, {}, { headers: getAuthHeaders() });
      fetchClients();
    } catch (err) {
      alert(err.response?.data?.error || texts.error);
    }
      ...
      <div className="pt-20 px-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-bold text-text-primary mb-2">{texts.clientManagement}</h1>
              <p className="text-text-secondary">{texts.manageClientsDesc}</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-surface rounded-lg shadow-sm p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-text-secondary">{texts.totalClients}</p>
            ...
            <div className="bg-surface rounded-lg shadow-sm p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-text-secondary">{texts.activeClients}</p>
            ...
            <div className="bg-surface rounded-lg shadow-sm p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-text-secondary">{texts.selectedClient}</p>
                  <p className="text-sm font-medium text-text-primary">
                    {localStorage.getItem('selectedClientId') ? texts.selected : '-'}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-surface rounded-lg shadow-sm p-4 mb-6">
          <div className="flex gap-4 items-center">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={texts.searchClientsPlaceholder}
            ...
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-text-secondary">{texts.activeOnly}</span>
            </label>
          </div>
        </div>
        ...
        {loading ? (
          <div className="bg-surface rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="mt-4 text-text-secondary">{texts.loadingClients}</p>
          </div>
        ) : error ? (
          ...
          <div className="bg-surface rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-text-secondary mb-4">{texts.noClientsFound}</p>
            <button
              onClick={() => setShowModal(true)}
              className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-blue-700"
            >
              {texts.addFirstClient}
            </button>
          </div>
        ) : (
              ...
              <div key={client.id} className="bg-surface rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        ...
                        {client.isActive ? texts.active : texts.inactive}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-4 border-t border-border-light">
                  ...
                  <button
                    ...
                    title={client.isActive ? texts.deactivate : texts.activate}
                  >
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-bold text-text-primary">
                    {editingClient ? texts.editClient : texts.addNewClient}
                  </h3>
                  <button
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-text-primary mb-2">{texts.firstName} *</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-text-primary mb-2">{texts.lastName} *</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">{texts.emailAddress} *</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">{texts.phoneNumber}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">{texts.company}</label>
                    <input type="text" name="company" value={formData.company} onChange={handleInputChange}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-accent" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">{texts.address}</label>
                    <textarea name="address" value={formData.address} onChange={handleInputChange} rows={2}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-accent" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-text-primary mb-2">{texts.notes}</label>
                  ...
                  <div className="flex gap-3 pt-4 border-t border-border-light">
                    ...
                    <button type="submit"
                      ...
                      {editingClient ? texts.updateClient : texts.createClient}
                    </button>
                  </div>
                </form>
              </div>
